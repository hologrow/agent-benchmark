...
export default function EvaluatorsPage() {
  ...
  const [deletingEvaluator, setDeletingEvaluator] = useState<Evaluator | null>(null);
  ...
  const onSubmit = async (values: FormData) => {
    try {
      ...
      const url = editingEvaluator ? `/api/evaluators/${editingEvaluator.id}` : '/api/evaluators';
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-2">配置 Benchmark 结果评估器，支持变量引用上下文</p>
        </div>
              ...
              <TableBody>
                {evaluators.map((evaluator) => (
                  <TableRow key={evaluator.id}>
                    ...
                    <TableCell className="max-w-xs truncate">{evaluator.description || '-'}</TableCell>
                    <TableCell className="font-mono text-sm">{evaluator.script_path}</TableCell>
                    <TableCell>{formatDate(evaluator.created_at)}</TableCell>
                    <TableCell className="text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Button variant="ghost" size="sm" onClick={() => handleEdit(evaluator)}>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editingEvaluator ? '编辑评估器' : '新建评估器'}</DialogTitle>
            <DialogDescription>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="如：scripts/run_evaluator.py" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                      ...
                      <FormControl>
                        <SelectTrigger>
                          <SelectValue placeholder="选择用于评估的 LLM 模型">
                            {field.value ? models.find(m => m.id === field.value)?.name : "选择用于评估的 LLM 模型"}
                          </SelectValue>
                        </SelectTrigger>
                      </FormControl>
                      <SelectContent>
                    ...
                    <FormDescription>
                      选择模型管理中配置的模型，评估时将使用该模型的 API 密钥和配置
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
                    ...
                    <FormDescription className="mb-2">
                      <div className="space-y-2">
                        ...
                        <ul className="list-disc pl-4 space-y-1 text-sm">
                          <li><strong>model</strong>：评估使用的 LLM 模型（如 claude-opus-4-6, claude-sonnet-4-6, claude-haiku-4-5-20251001）</li>
                          <li><strong>max_workers</strong>：并行评估的进程数</li>
                          <li><strong>variables</strong>：自定义变量，可在 evaluation_prompt 中使用</li>
              ...
              <DialogFooter>
                <Button type="submit">{editingEvaluator ? '保存' : '创建'}</Button>
              </DialogFooter>
            </form>
          </Form>
        </DialogContent>
