...
-- Models 表：存储 LLM 模型配置和授权信息
CREATE TABLE IF NOT EXISTS models (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    name TEXT NOT NULL UNIQUE, -- 显示名称，如 "Claude Sonnet"
    model_id TEXT NOT NULL, -- 模型标识，如 "claude-sonnet-4-6"
    provider TEXT DEFAULT 'anthropic', -- 提供商：anthropic, openai, azure 等
    api_key TEXT, -- API 密钥
    base_url TEXT, -- 自定义 API 基础 URL（可选）
    config TEXT, -- JSON 格式，额外配置参数（温度、max_tokens 等）
